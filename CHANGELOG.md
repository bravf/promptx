...
- Android WebView 客户端支持长按预览图调起系统分享：客户端会识别 WebView 中的图片长按，将当前图片转为临时 PNG 并通过只读 content provider 分享给其它 App，兼容 `blob:` 生成图。
- Android WebView 客户端上传图片时补齐拍照入口：客户端会先弹出“拍照 / 选择文件”原生选择框，避免部分手机系统文件选择器不展示相机选项；拍完后自动把临时 JPEG 回传给网页上传控件。
...
- 继续打磨主题一致性：修复 `Aqua Classic` 下 Agent 选择器、任务 / 项目选中态、危险按钮颜色等细节，并同步补齐 `Glass Light` 下 Agent 选择器的圆角和玻璃质感。
- 优化设置页通用表单：语言选择从下拉改为卡片式单选，发送消息方式的单选控件统一放到右侧，让 radio / checkbox 的布局保持一致。
...
- 新增工作台 `shell` 命令模式：在右侧编辑区输入 `!pwd`、`!git status`、`!pnpm test` 这类纯文本命令时，PromptX 会直接按当前选中的 `Codex / Claude Code / OpenCode` 语义发起本地命令执行，并把结果继续收拢回原有的 `PromptX → Agent / Agent` 会话流，不额外引入一套独立的 Shell 卡片心智。
...
- 补齐常用样式 / 配置文件的语言识别与高亮：新增 `SCSS / Sass / Less / TOML / INI / .env / C#` 的源码预览与回复代码块语言映射，解决 `.scss` 等文件明明是代码却仍按纯文本展示的问题。
- 为上述语言识别补充前后端回归测试，并再次确认前端构建无大包告警，保证这轮补丁发布足够稳定。

## 0.1.43

- 继续完善项目源码查看：左侧目录树、路径搜索、内容搜索、`@` 文件选择器与代码变更文件列表统一为同一套选中 / hover / 间距 / 图标 / 副文本视觉语言，深浅主题下的状态反馈更一致。
- 项目源码查看补齐键盘与搜索体验：支持 `↑ / ↓ / ← / → / Enter / Esc / Tab` 的连续操作，输入框默认接管焦点；内容搜索结果改为按文件分组展示，右侧预览可同步滚到命中行，并区分“命中行 / 当前命中行 / 命中关键词”三层高亮。
- 优化源码预览性能：同一文件内多处命中切换不再重复请求完整内容，弹窗内增加最近使用预览缓存，减少内容搜索场景下的重复加载与闪烁。
- 新增项目只读源码浏览链路与接口：项目管理支持直接打开源码弹层，服务端补充工作区目录树、路径搜索、内容搜索与文件内容读取能力，并补齐相关前后端测试。
...
- 为回复卡片新增代码高亮能力：`response` 区域现在支持 `JavaScript / TypeScript / React / Vue / JSON / Bash / Python / SQL / Java / HTML / CSS / Diff / Markdown` 等常见代码块高亮，并补充轻量语言标签与相关回归测试。
- 收紧回复代码块视觉细节：压缩代码块内边距和语言标签尺寸，修复标签遮挡首行代码的问题，同时加强 `WeChat` 主题下代码块边框对比度，避免边框在浅灰背景里“化掉”。
- 彻底拆分 `response` 与 `process detail` 的 Markdown 渲染策略：回复继续保留代码高亮，执行过程详情则回退为普通 Markdown/文本，不再做代码高亮识别，减少误判和过程区样式干扰。

## 0.1.38

- 统一中栏消息主卡片边框为实线，弱化虚线带来的割裂感，让 `PromptX / 执行 / 回复` 三类卡片视觉更稳定。
- 重新收敛中栏消息间距：卡片组外层与单轮内部卡片统一改为同一档间隔，并同步压平执行卡片内部块间距，避免出现“有的松有的紧”的不一致观感。
...
- 收回执行过程区域的等宽字体策略：执行卡正文、过程详情、命令路径与文件变更列表统一改回普通文本字体，避免中栏阅读密度和其它消息卡片割裂。

## 0.1.35

- 收敛中栏消息卡片布局：`PromptX / 执行 / 回复` 三类主卡片统一为满宽展示，标题样式与时间信息位置更一致，阅读节奏更接近标准会话流。
- 继续简化中栏细节：`PromptX` 附图改为直接展示图片本身，不再额外挂一层“本轮附图”卡片；项目选择器加载文案也收短为 `加载... / Loading...`。
- 统一消息卡片边框语义：中栏主卡片的虚线边框统一使用 `--theme-borderDefault`，同时补齐毛玻璃主题下部分内容面板的圆角一致性，避免局部观感发飘。
...
- 调整中栏消息头文案，统一收敛为 `PromptX：` 与 `{agent}：`，让执行区更接近会话阅读体验。
