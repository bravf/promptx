...
PromptX 采用 Codex App 风格的任务级隔离模型：每条会话都是一个独立开发任务，每个任务拥有自己的执行环境；Worktree 是执行环境的一种实现。

这次升级允许直接重建数据模型，不保留旧 workspace 数据迁移兼容。界面保留当前工作台习惯：左栏继续展示工作区和会话，Timeline 继续作为任务主界面，Worktree、分支和 Git 状态集中在右侧任务详情抽屉中。
...
| 会话 / 任务 | Task | 一次独立开发任务和一条 Timeline |
...
Worktree 信息只允许以轻量辅助信息出现在会话行，例如分支名；完整路径、基线和 Git 状态不放进左栏。
...
如果历史目录存在，使用原目录并标记 `ownership = external`。如果历史目录不存在，仍然导入会话，Environment 标记为 `unavailable`；用户可以查看历史，但暂时不能继续执行 Agent。
