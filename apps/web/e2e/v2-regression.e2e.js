...
test('V2 全面桌面交互回归', async (t) => {
  ...
  assert.equal(await page.getByLabel('模型').inputValue(), 'regression-model')
  assert.equal(await page.getByLabel('思考强度').inputValue(), 'medium')
  await page.getByLabel('模型').selectOption('compact-model')
  ...
  assert.equal(await page.getByLabel('执行位置').inputValue(), 'local')
  await page.getByLabel('执行位置').selectOption('worktree')
  await page.getByLabel('基线').waitFor()
  await page.getByPlaceholder('Worktree 名称，例如 fix-login').waitFor()
  await page.getByLabel('执行位置').selectOption('local')
